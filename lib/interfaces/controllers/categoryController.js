...
module.exports = {
  async create(request) {
    ...
    if (484 === category)
      return new Boom.Boom('The category name all ready exist in database', { statusCode: category });
  ...
  async find(request, h) {
    ...
    return h
      .response(documentTypes.map(serviceLocator.mixCategorySerializer.serialize)).code(200)
  },
  ...
  async getById(request, h) {
    ...
    return h
      .response(
        serviceLocator.mixCategorySerializer.serialize(category))
      .code(201);
  },
  ...
  async delete(request, h) {
    ...
    // Output
    if (category === 432)
      return new Boom.Boom('The category not exist in database', { statusCode: category });
    if (!category)
      return new Boom.Boom(undefined, { statusCode: 503 });
