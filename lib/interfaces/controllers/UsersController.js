...
module.exports = {
  async create(request) {
    ...
    // Input
    const { document, roleId, documentTypeId, name, lastName, email, dateBirthday, } = request.payload;

    // Treatment
    const user = await createUser(document, roleId, documentTypeId, name, lastName, email, dateBirthday, serviceLocator);

    // Output
    if (484 === user)
      return new Boom.Boom('The user email all ready exist in database', { statusCode: user });
    if (485 === user)
      return new Boom.Boom('The document all ready exist in database', { statusCode: user });
  ...
  async find(request, h) {
    ...
    return h
      .response(users.map(serviceLocator.authUserSerializer.serialize)).code(200)
  },
  ...
  async getById(request, h) {
    ...
    return h
      .response(
        serviceLocator.authUserSerializer.serialize(user))
      .code(201);
  },
  ...
  async delete(request, h) {
    ...
    // Output
    if (user === 432)
      return new Boom.Boom('The user not exist in database', { statusCode: user });
    if (!user)
      return new Boom.Boom(undefined, { statusCode: 503 });
