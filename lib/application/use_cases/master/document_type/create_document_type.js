...
const { generateCurrentDate, convertUpperCase } = require('../../../utilities/general_functions') 


module.exports = async (name, description, { masterDocumentTypeRepository }) => {
  name = await convertUpperCase(name);
  //Validate that a record with the same name is not found.
  const validationName = await masterDocumentTypeRepository.getByFilter({name});
  ...
  const resultCreateDocumentType = await masterDocumentTypeRepository.persist(documentType);
